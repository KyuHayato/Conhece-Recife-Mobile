const parceirosMongo = require("../models/parceiros.js");
const parceiros = require("../models/parceiros.js");
...
const getByBairro = (req, res) => {
  ...
  parceirosMongo.find(
    { bairro },
    "nome categoria cidade rua estado complemento cep bairro numero cupom tipoCupom descricao telefone  ",
    function (err, parceiros) {
      if (err) {
        res.status(500).send("Não temos o registro");
      } else return res.status(200).send(parceiros);
    }
  );
};
  ...
  try {
    parceirosMongo.find({ id }, function (err, parceiros) {
      if (parceirosMongo.length > 0) {
        ...
        res.status(200).send({
          message: "Não há parceiro para ser removido",
          status: "EMPTY",
        });
    });
  } catch (err) {
    return res
      .status(424)
      .send({ message: "Erro ao deletar o registro do parceiro" });
  }
};

const putParceiros = (req, res) => {
  const id = req.params.id;
  parceirosMongo.updateMany(
    { id },
    { $set: req.body },
    { upsert: true },
    function (err) {
